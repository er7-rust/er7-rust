...
than naming a position ([§6.3](06-policy-file-format.md)).
...
fails safely ([§1.5](01-purpose-and-scope.md), priority 1). The wrong
answer is recoverable by editing the policy; the other one is not
recoverable at all.
...
built by naming one ([§5](05-built-in-policies.md)), a policy file writes
one out ([§6.5](06-policy-file-format.md)), and `--show-policy` prints it,
so "which of the two is this?" is always answerable without reading code.
...
one policy ([§10.2](10-command-line-interface.md)), and `Policy::append`
is how. Rules are appended in order, and the posture is combined by taking
...
A policy file that says nothing about its posture accepts by default, and
a file of extra rules is exactly such a file. If appending adopted its
...
and did not, which is the one failure [§1.5](01-purpose-and-scope.md)
puts first. So it cannot happen by accident: weakening a posture is done
deliberately, with `Policy::posture` or the CLI's `--accept-all`
([§10.2](10-command-line-interface.md)).
...
payload is given `refuse` when it is read ([§6.1](06-policy-file-format.md))
— the strictest disposition there is — so every value one carries is
somebody's decision, and a file that went to the trouble of writing
`unrecognised pass` is not overruled by a default it never saw.
...
| `Policy::patient_identifiers()` | refuse | it is a *list of positions*, and it has no opinion about a payload with no positions in it. Refusing is fail-closed: nothing is written, and a person decides ([§10.4](10-command-line-interface.md)) |
| `Policy::all_but_the_header()` | refuse | as above |

Any of them is overridden with `Policy::on_unrecognised`, or the
`unrecognised` line of a policy file ([§6.3](06-policy-file-format.md)).

Refusing is not an error the *library* raises: `Redactor::redact` takes a
parsed message and cannot fail ([§9.2](09-error-handling.md)), and
...
does ([§10.4](10-command-line-interface.md)).

Masking a payload whole preserves its length, and nothing else. That is
the same leak `Mask` always has ([§5.5](05-built-in-policies.md)), applied
to a bigger value: use `clear` or `replace` where the size of what was
dropped is itself worth hiding.
