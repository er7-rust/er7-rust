[`er7-redact` specification](../index.md) — section 5 of 17. Section
numbers (§5.x) are stable and cited from code, tests, and commit messages.
...
cannot exist without a posture ([§2.6](../02-redaction-model/index.md)), so
there is no `Policy::new` and no `Policy::default` to make one without
saying which posture is meant.
...
The header exception is an ordinary accept rule, so an ordinary reject rule
overrides it (D19, [§2.4](../02-redaction-model/index.md)) — this redacts
the sending facility too, and everything else `MSH-3` and beyond carries:
...
delimiters themselves (D5,
[§4.4](../04-what-redaction-preserves/index.md)).
...
- **leak-free by construction.** `Mask` preserves length. `First(4)` on a
  ...
  possible ([§7.3](../07-pseudonyms/index.md)).
...
`accept_all()` is the starting point for building a policy up rule by rule;
it is what `Policy::parse` starts from, and what the CLI applies `--policy`
and `--rule` on top of ([§10.2](../10-command-line-interface/index.md)). On
its own it is a no-op, and an honest one: it does not pretend to have
redacted anything, and it passes a payload it cannot parse through
unchanged rather than mangling it ([§2.8](../02-redaction-model/index.md)).
