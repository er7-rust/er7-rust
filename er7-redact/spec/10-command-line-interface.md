...
policy refuses ([§2.8](02-redaction-model.md)) fails the run rather than
producing a half-redacted output, however late in a batch it is.
...
| `--reject-all` | start from `Policy::reject_all()`: reject every value, header included ([§5.6](05-built-in-policies.md)) |
| `--all-but-the-header` | start from `Policy::all_but_the_header()`: reject every value but keep `MSH` ([§5.2](05-built-in-policies.md)). This is what `--all` did |
...
| `--reject-all` | `Policy::reject_all()` ([§5.6](05-built-in-policies.md)) |
| `--all-but-the-header` | `Policy::all_but_the_header()` ([§5.2](05-built-in-policies.md)) |
...
`accept_all` passes one ([§2.8](02-redaction-model.md)). A run handed
nothing but rules has been told nothing about what an unreadable payload is
worth, and refusing is the disposition that loses no value quietly. Ask for
`unrecognised pass` in a policy file to get the other one — a file that
states a disposition is not overruled (D20).
...
[§2.4](02-redaction-model.md)). Appending never weakens the defaults the
starting policy came with (D20, [§2.6](02-redaction-model.md)): a policy
file of extra rules cannot turn `--reject-all` back off.
...
replaced it are named for the two postures ([§2.6](02-redaction-model.md)).
A script that kept working but had switched posture is the one outcome
worth ruling out.
...
([§2.8](02-redaction-model.md)) has no positions, and so contributes no
rows. It gets a comment line instead, which is not a change row and cannot
be mistaken for one:
...
one** ([§2.8](02-redaction-model.md)), which the two curated policies do,
and so does every run that names none. A policy that passes such a payload
through, or masks it whole, exits 0 and writes it — the run did what the
policy said. This is the one place a CLI diagnostic depends on the policy
...
report's layout is a breaking change. Adding an option is not. Removing
