[`er7-redact` specification](../index.md) — section 8 of 17. Section
numbers (§8.x) are stable and cited from code, tests, and commit messages.
...
A rule that names a field with three components produces three rows — one
per leaf that actually changed. A rule that matched nothing produces none
(D8, [§2.5](../02-redaction-model/index.md)). A position that two rules
both named produces two rows, in the order the rules ran (D7), which is
what makes an unintended overlap visible.
...
- **Regression.** Assert on it in a test. A report is stable output (D7,
  [§2.7](../02-redaction-model/index.md)), so a change to the policy shows
  up as a diff.
...
[§5.5](../05-built-in-policies/index.md).
