[`er7-redact` specification](../index.md) — section 1 of 17. Section
numbers (§1.x) are stable and cited from code, tests, and commit messages.
...
([§10](../10-command-line-interface/index.md)) is a thin presentation layer
...
| Stable pseudonyms, so an identifier maps the same way in every message | [§7](../07-pseudonyms/index.md) |
| An audit report of every position that changed | [§8](../08-report/index.md) |
...
reused.** [§11.1](../11-testing-strategy/index.md) maps each rule to the
test that enforces it.

| ID | Rule | Section |
| -- | ---- | ------- |
| D1 | Redaction rewrites leaf text only; the shape of the message is preserved. | [§4.1](../04-what-redaction-preserves/index.md) |
| D2 | Redaction never creates a position the message did not carry. | [§4.2](../04-what-redaction-preserves/index.md) |
| D3 | A leaf that carried no text is left empty; redaction never invents a value. | [§4.3](../04-what-redaction-preserves/index.md) |
| D4 | An explicit null stays null; redaction never turns "clear this" into a value. | [§4.3](../04-what-redaction-preserves/index.md) |
| D5 | The header's delimiter fields are never redacted, by any rule. | [§4.4](../04-what-redaction-preserves/index.md) |
| D6 | `Null` is the only action that changes shape: it collapses the named position to `""`. | [§3.4](../03-actions/index.md) |
| D7 | Rules apply in order, each to the message as it stands. | [§2.4](../02-redaction-model/index.md) |
| D8 | A rule that matches nothing is not an error. | [§2.5](../02-redaction-model/index.md) |
| D9 | A policy accepts or rejects by default; rejecting applies its action to every leaf no rule named. | [§2.6](../02-redaction-model/index.md) |
| D10 | Every action except `Pseudonym` is idempotent. | [§3.6](../03-actions/index.md) |
| D11 | Replacement text is encoded on the way in, so redaction can never introduce a delimiter. | [§3.5](../03-actions/index.md) |
| D12 | A pseudonym is stable for a given key and value, and is not a cryptographic guarantee. | [§7](../07-pseudonyms/index.md) |
| D13 | A report never contains the text that was redacted. | [§8.2](../08-report/index.md) |
| D14 | The built-in policies are a starting point, not a compliance certification. | [§1.3](index.md), [§5](../05-built-in-policies/index.md) |
| D15 | Errors arise from exactly two situations: a policy that cannot be read, and a path that is not a path. | [§9](../09-error-handling/index.md) |
| D16 | Exactly one runtime dependency: `er7`. | [§12](../12-dependencies-and-build/index.md) |
| D17 | A message no rule touches is written back byte for byte. | [§4.5](../04-what-redaction-preserves/index.md) |
| D18 | The policy file format is one rule per line, and it is part of the public API. | [§6](../06-policy-file-format/index.md) |
