...
Standard SemVer governs this crate (currently pre-1.0, so the usual "minor
may break" caveat of the `0.y.z` range applies per the Cargo/SemVer
convention — see [`AGENTS/release.md`](../../AGENTS/release.md)). What
makes this crate slightly unusual is that its *wire shape* —
[§2](../02-wire-shapes/index.md)'s table — is as much a compatibility
surface as its Rust API: a caller who has stored `Message` JSON on disk, in
a database, or in a message queue is depending on that shape, not merely on
the Rust types that produce it.
...
- Changing `Subcomponent` to serialize `value()` instead of `raw` (this
  would also violate [§4](../04-round-trip-guarantee/index.md), so it
  should never happen without a spec change to that section too, with its
  rationale addressed head-on).
...
| ---- | ----- |
| Edition | 2024 |
| MSRV | 1.96 — current stable minus two releases |
...
is below that and no longer binds.
...
`serde` ever declares an MSRV above `N-2`, that becomes the real floor and
