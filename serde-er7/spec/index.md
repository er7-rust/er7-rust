...
— it only specifies the one thing this crate adds: Serde support for the
tree `er7` already defines.

## Sections

| § | Title | Covers |
|---|-------|--------|
| [1](01-purpose-and-scope.md) | Purpose and scope | What this crate is, the rule index, which goal wins when two conflict |
| [2](02-wire-shapes.md) | Wire shapes | The exact `Serialize`/`Deserialize` shape for every level of the tree |
| [3](03-dependencies-and-format-agnosticism.md) | Dependencies and format-agnosticism | Why exactly two dependencies, and why no format is named in the library code |
| [4](04-round-trip-guarantee.md) | The round-trip guarantee | What survives a Serde round trip, and the one thing that deliberately does not |
...
|----|---------------------|---------|
| S1 | Exactly two runtime dependencies: `serde` and `er7` | [§3](03-dependencies-and-format-agnosticism.md) |
| S2 | No format-specific crate is a runtime dependency | [§3](03-dependencies-and-format-agnosticism.md) |
| S3 | A subcomponent serializes as its `raw` text, never `value`-decoded | [§2](02-wire-shapes.md), [§4](04-round-trip-guarantee.md) |
| S4 | Field, Repetition, and Component serialize as bare arrays, not objects | [§2](02-wire-shapes.md) |
| S5 | Message and Segment serialize as objects with named fields | [§2](02-wire-shapes.md) |
| S6 | A `char` delimiter serializes via `serialize_char`, not as part of a larger string | [§2](02-wire-shapes.md) |
| S7 | `Terminator` serializes as its Rust variant name, as a string | [§2](02-wire-shapes.md) |
| S8 | Deserializing an object ignores unknown fields rather than rejecting them | [§5](05-error-handling.md) |
| S9 | A missing required field is a `missing_field` error naming that field | [§5](05-error-handling.md) |
| S10 | The wire shape in [§2](02-wire-shapes.md) is part of this crate's public API and its SemVer contract | [§8](08-versioning-and-compatibility.md) |
| S11 | Every wrapper type implements `Deref`, `DerefMut`, and `From` both ways | [§6](06-ergonomics.md) |
| S12 | Every public item carries a doc comment; `cargo rustdoc --lib -- -W missing-docs` stays clean | [§7](07-testing-strategy.md) |
