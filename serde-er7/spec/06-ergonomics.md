...
Unlike the shapes in [§2](02-wire-shapes.md), `Deref`/`DerefMut`/`From` are
Rust-side ergonomics with no wire representation — removing or adding one
does not change what bytes a `Serialize` call produces or a `Deserialize`
call accepts. They are still part of this crate's public API, and removing
one is still a breaking change under normal SemVer rules ([§8](08-versioning-and-compatibility.md)),
but they are tracked separately from S10 because a wire-shape change and an
ergonomics change call for different migration advice to downstream users.
