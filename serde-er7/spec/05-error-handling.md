...
- **Serde-level failure** — the *Serde* input (JSON, YAML, ...) does not
  match the shape in [§2](02-wire-shapes.md): a required key is missing, a
  string was expected where an array appeared, and so on. This is reported
  through the format's own error type (`D::Error`/`S::Error`), via the
  standard `serde::de::Error`/`serde::ser::Error` constructors.
...
kind. A logic error in this crate's own code (an index out of bounds while
...
lints, both required by the checks in [the index](index.md#required-checks),
are the mechanical backstop for this.
