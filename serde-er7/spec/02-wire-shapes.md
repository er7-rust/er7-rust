...
change (see [§8](08-versioning-and-compatibility.md), rule S10).
...
- **S3**: `Subcomponent` serializes `raw`, the text exactly as the sender
  ...
  [§4](04-round-trip-guarantee.md) possible: decoding is lossy (an escaped
  formatting instruction such as `\.br\` has no plain-text form to decode
  back to), so any wire format built on the decoded value could not, in
  general, be turned back into the original ER7 text.
