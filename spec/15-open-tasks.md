[`er7-redact` specification](index.md) — section 15 of 17. Section numbers (§15.x) are stable and cited from code, tests, and commit messages.
...
biggest gap in a positional redactor
([§14.2](14-roadmap.md), [§5.4](05-built-in-policies.md)).
...
**Why.** `Action` is closed, so a real MAC ([§7.4](07-pseudonyms.md)), a
lookup table, or a date shift cannot be expressed at all.
...
**Why.** [§7](07-pseudonyms.md) asserts that collisions are negligible at
the scale a redaction run works over. That is a property of FNV-1a rather
than of this crate, and it is currently argued rather than shown
...
pass over a small message ([§11.6](11-testing-strategy.md)).
...
which is what longitudinal test data is for ([§14.4](14-roadmap.md)).
...
named by no rule — the set the fallback already computes — and a CLI flag
to print it.
