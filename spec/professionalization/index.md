...
Assessed 2026-08-26, updated 2026-08-27, 2026-08-30, and 2026-09-02, by
reading the tree rather than remembering it.
...
| 4 | **Met** | [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) runs the four checks, an MSRV build (1.96 as of 2026-08-29, read dynamically from `er7/Cargo.toml` rather than a second hard-coded pin — checked against the manifests 2026-08-30, not assumed unchanged since the last pass), a fuzz-target build-and-smoke job, `bin/check-trademarks`, and (since 2026-08-27) `cargo deny` — on every push and pull request. Checked against `gh run list -R er7-rust/er7-rust --workflow=ci.yml --limit 200 --json conclusion` 2026-09-02, not the 2026-08-30 figure this row previously carried: 44 hosted runs of this workflow, 43 green; the one failure (`61eb30d`) was a flaky CLI-test race, root-caused and fixed the same day (`574daaf`). A real, growing track record now, not a single proof-of-concept run — and, checked four times over the course of this one day alone (34, 39, 43, 44 runs), it is worth reading this figure as "still climbing," not as a number to expect fixed |
| 5 | **Met** | `bin/check-trademarks` passes across the tree (run 2026-09-02); the 100+-file sweep extending compliance into crate doc-comments, manifests, and site copy landed 2026-08-26 and reached crates.io with each crate's very next release — `er7` 0.2.1, `serde-er7` 0.2.0, and `er7-redact` 0.3.0, all published 2026-08-29 — confirmed again by `serde-er7` 0.3.0 and `er7-redact` 0.4.0, both published 2026-09-02. Nothing published today predates the sweep |
