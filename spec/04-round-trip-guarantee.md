...
Rule S3 ([§2.2](02-wire-shapes.md)): every subcomponent serializes its
`raw` field, not the escape-decoded `value()`. Decoding is lossy on
purpose — `er7`'s spec §6.2 keeps formatting escapes such as `\.br\` and
`\H\` undecoded because they say something a plain string cannot carry —
so a wire format built on the decoded form could not, in general, encode
back to the original bytes. Serializing `raw` sidesteps the problem
entirely: nothing is decoded, so nothing needs to be re-encoded.
...
covered by [§2](02-wire-shapes.md) and the per-level unit tests.
