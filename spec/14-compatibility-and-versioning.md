[`er7` specification](index.md) — section 14 of 19. Section numbers (§14.x) are stable and cited from code, tests, and commit messages.
...
by a second crate ([§16](16-roadmap.md)).

The **CLI is versioned with the library** ([§12.5](12-command-line-interface.md)):
removing an option, changing an exit code, or changing the outline's label
...
| adding a variant to `Error` | yes — callers match exhaustively ([§11.3](11-error-handling.md)) |
...
| changing what a rule in [§1.4](01-purpose-and-scope.md) guarantees | yes |
...
build messages from literals ([§5.1](05-value-tree.md)) — and it means the
struct shapes are part of the contract.
...
| ----- | ---------- | -------- |
| the truncation character | v2.7 | read from MSH-2 position 5 when present, `None` when not ([§3.1](03-delimiters.md)) |
| MSH-9.3 message structure | v2.3.1 | reported when present, never derived ([§10.3](10-msh-conveniences.md)) |
