[`er7-redact` specification](index.md) — section 17 of 17. Section numbers (§17.x) are stable and cited from code, tests, and commit messages.
...
| **de-identification** | removing enough that a person cannot reasonably be identified. A judgement about a data set, made by a person; not something this crate performs ([§1.3](01-purpose-and-scope.md)) |
| **redaction** | removing named values from one message. What this crate does |
| **pseudonym** | a stable stand-in for an identifier, so that equal values stay equal across messages ([§7](07-pseudonyms.md)) |
...
| **quasi-identifier** | a value that identifies nobody alone and identifies somebody in combination — a birth year, a postcode, a rare diagnosis ([§5.4](05-built-in-policies.md)) |
| **key** | the `u64` a pseudonym is derived from. Not a secret in any managed sense ([§7.3](07-pseudonyms.md)) |
