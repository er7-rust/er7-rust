[`er7` specification](index.md) — section 4 of 19. Section numbers (§4.x) are stable and cited from code, tests, and commit messages.
...
- **Nothing else is trimmed.** A value that really ended in a space keeps
  ...
  and here it is (see [§1.5](01-purpose-and-scope.md) priority 1).
...
segment; its delimiters ([§3.2](03-delimiters.md)) govern the whole
message.
...
alphanumeric ([§3.3](03-delimiters.md)), so the run ends precisely where
the name does. `MSHX|...` therefore has the name `MSHX`, not `MSH`.
...
[`split_messages`](09-batch-input.md), which borrows, and query the
resulting `&str` directly.
