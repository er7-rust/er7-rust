[`er7` specification](index.md) — section 18 of 19. Section numbers (§18.x) are stable and cited from code, tests, and commit messages.
...
([§2.5.1](02-er7-encoding.md)). This crate decodes sequences wherever they
appear.
...
[§10.3](10-msh-conveniences.md) lists what is deliberately absent: deriving
MSH-9.3, and accessors for MSH-3 through MSH-7 and MSH-11.

**Why they were declined:** each fails at least one of the two tests in
[§10.2](10-msh-conveniences.md) — universality and version-stability.
Deriving MSH-9.3 fails stability outright. The rest fail universality: they
are one `query` call away and not every tool needs them, so adding them
would grow the exception to R24 without earning it.
...
(R17), so its output is not re-parseable
([§7.1](07-writing.md)). That is documented, but it is still a shape that
invites misuse: a caller who reaches for `to_text` on a `Field` and writes
the result into a new message has silently corrupted it.
