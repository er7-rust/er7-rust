[`er7` specification](index.md) — section 9 of 19. Section numbers (§9.x) are stable and cited from code, tests, and commit messages.

# 9. Batch and multi-message input

Implemented in `src/parse.rs`. Background in
[§2.6](02-er7-encoding.md).
...
[`parse`](04-parsing.md) with no copy and no change of meaning. This is why
the return type is `Vec<&str>` rather than `Vec<String>`.
...
([§4.2](04-parsing.md)) — so a local segment such as `BTSX` is not mistaken
for a batch trailer. The three-letter prefix alone would not be safe.
...
`split_messages` does **not** unframe MLLP ([§2.7](02-er7-encoding.md)).
Input is expected to be text; a caller reading from a socket strips the
0x0B / 0x1C 0x0D framing bytes first. Transport is out of scope (R24).
...
type this crate does not want to grow ([§11](11-error-handling.md)).
