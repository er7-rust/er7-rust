[`er7` specification](../index.md) — section 11 of 19. Section numbers
(§11.x) are stable and cited from code, tests, and commit messages.
...
| `BadHeader(detail)` | the declared delimiters are unusable ([§3.3](../03-delimiters/index.md)) | `parse`, `Separators::from_header`, `Separators::validate` |
| `BadPath(detail)` | a path could not be read ([§8.1](../08-paths-and-queries/index.md)) | `Path::parse`, `FromStr for Path`, `Message::query`, `Message::query_all` |
...
one that reads it as written ([§1.5](../01-purpose-and-scope/index.md)
priority 3).
...
| an undecodable `\X..\`, or an unknown `\..\` | kept literally ([§6.2](../06-escape-sequences/index.md)) |
| an unterminated escape character | kept literally |
| a query naming a position that is absent | contributes no value (R20) |
| a batch `BTS` count disagreeing with reality | not checked ([§9.4](../09-batch-input/index.md)) |

`parse_with` ([§4.3](../04-parsing/index.md)) has no failure mode at all,
and so returns `Message` rather than `Result<Message, Error>`.

## 11.3 Adding a variant

Adding a variant is a **breaking change** for callers that match
exhaustively ([§14](../14-compatibility-and-versioning/index.md)), and it
moves a situation out of the "recovered" column above. Both need justifying
in the same change, and both need §11.1 and §11.2 updated. The enum is
deliberately not marked `#[non_exhaustive]`: four variants that are
unlikely to grow are more useful matched exhaustively than guarded by a
wildcard arm that hides a future case.
