[`er7` specification](../index.md) — section 1 of 19. Section numbers
(§1.x) are stable and cited from code, tests, and commit messages.
...
library is the contract; the CLI
([§12](../12-command-line-interface/index.md)) is a thin presentation layer
over it and adds no behaviour of its own.

## 1.2 In scope

| Capability | Section |
| ---------- | ------- |
| Reading a message's own delimiter set, including the v2.7 truncation character | [§3](../03-delimiters/index.md) |
| Parsing text into the six-level value tree | [§4](../04-parsing/index.md), [§5](../05-value-tree/index.md) |
| Distinguishing absent, empty, and explicit-null values | [§5.3](../05-value-tree/index.md) |
| Tokenizing, decoding, and encoding escape sequences | [§6](../06-escape-sequences/index.md) |
| Writing the tree back out, byte for byte | [§7](../07-writing/index.md) |
| Reading values by HL7 path (`PID-5.1`, `OBX[2]-5`) | [§8](../08-paths-and-queries/index.md) |
| Splitting batch files and concatenated messages | [§9](../09-batch-input/index.md) |
| Five `MSH` routing accessors | [§10](../10-msh-conveniences/index.md) |
...
considered, are in [§18.1](../18-open-questions-and-divergences/index.md).
...
onto `er7` is what [§16.3](../16-roadmap/index.md) records.
...
reused.** [§13.1](../13-testing-strategy/index.md) maps each rule to the
test that enforces it.

| ID | Rule | Section |
| -- | ---- | ------- |
| R1 | Delimiters are read from the message's own header, never assumed. | [§3.2](../03-delimiters/index.md) |
| R2 | A delimiter set with an alphanumeric, line-ending, or repeated character is rejected. | [§3.3](../03-delimiters/index.md) |
| R3 | Encoding characters the sender omitted fall back to their recommended values. | [§3.2](../03-delimiters/index.md) |
| R4 | Segments divide at `\r`, `\n`, or `\r\n`; blank lines are dropped; nothing else is trimmed. | [§4.1](../04-parsing/index.md) |
| R5 | `parse` requires the first segment to be `MSH`, `FHS`, or `BHS`. | [§4.2](../04-parsing/index.md) |
| R6 | Nothing below the header can fail. | [§4.2](../04-parsing/index.md) |
| R7 | A field the sender left empty has zero repetitions. | [§4.4](../04-parsing/index.md) |
| R8 | Header fields 1 and 2 are the delimiters themselves, stored whole and never decoded. | [§4.4](../04-parsing/index.md) |
| R9 | Leaf text is stored exactly as sent and decoded only on demand. | [§5.2](../05-value-tree/index.md) |
| R10 | Absent, empty, and explicit null are three distinct states. | [§5.3](../05-value-tree/index.md) |
| R11 | `is_empty` and `is_null` are never both true. | [§5.3](../05-value-tree/index.md) |
| R12 | Tokenizing escape sequences is lossless and never fails. | [§6.1](../06-escape-sequences/index.md) |
| R13 | `unescape` resolves only the sequences that stand for characters; the rest stay literal. | [§6.2](../06-escape-sequences/index.md) |
| R14 | `escape` encodes the five structural delimiters, plus `\r` and `\n` as hex. | [§6.3](../06-escape-sequences/index.md) |
| R15 | `unescape(escape(value)) == value` for every value. | [§6.3](../06-escape-sequences/index.md) |
| R16 | Writing reproduces canonical parsed input byte for byte. | [§7.2](../07-writing/index.md) |
| R17 | Structural delimiters remain in `to_text`; only leaf text is decoded. | [§7.1](../07-writing/index.md) |
| R18 | Path indices are 1-based, and `0` is rejected rather than clamped. | [§8.1](../08-paths-and-queries/index.md) |
| R19 | An omitted segment occurrence or repetition index matches every one. | [§8.2](../08-paths-and-queries/index.md) |
| R20 | A position the message does not have yields no value, not an error and not an empty string. | [§8.2](../08-paths-and-queries/index.md) |
| R21 | `split_messages` starts a message at each `MSH` and drops batch envelope segments. | [§9](../09-batch-input/index.md) |
| R22 | The MSH conveniences return `None` when the position is absent or empty. | [§10](../10-msh-conveniences/index.md) |
| R23 | Exactly four error variants exist, arising from exactly two situations. | [§11](../11-error-handling/index.md) |
