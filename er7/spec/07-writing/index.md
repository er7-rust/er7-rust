[`er7` specification](../index.md) — section 7 of 19. Section numbers
(§7.x) are stable and cited from code, tests, and commit messages.
...
| `to_text(&separators)` | the same, with leaf text escape-decoded per [§6.2](../06-escape-sequences/index.md) |
...
([§4.4.2](../04-parsing/index.md)), since they are the delimiters rather
than values encoded with them.
...
output; the CLI's `--normalize` does
([§12](../12-command-line-interface/index.md)).
...
exactly those two things ([§4.1](../04-parsing/index.md)) and nothing else.
...
crate's first design priority ([§1.5](../01-purpose-and-scope/index.md))
and the property most likely to be broken by a well-meaning change — see
