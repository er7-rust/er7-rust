[`er7` specification](index.md) — section 5 of 19. Section numbers (§5.x) are stable and cited from code, tests, and commit messages.
...
Six types, one per level of [§2.2](02-er7-encoding.md):
...
`Repeat`; see [§18.3](18-open-questions-and-divergences.md).)

## 5.2 Text lives only at the leaf [R9]

Only `Subcomponent` holds text, in its `pub raw: String` field, exactly as
sent. Decoding happens on demand
([§6.2](06-escape-sequences.md)). This single decision is what makes the
round-trip guarantee of [§7.2](07-writing.md) possible, and it keeps
separate the two questions a receiver asks:
...
ER7 distinguishes three states ([§2.4](02-er7-encoding.md)), and so does
this crate.
...
| `subcomponent.set(value, &separators)` | the crate encodes delimiters for you ([§6.3](06-escape-sequences.md)) |
