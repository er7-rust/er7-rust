...
The operational checklist is [`help/releasing/`](../help/releasing/index.md);
this is the policy behind it.

## Versioning

Semantic versioning, over a public surface that is larger than the Rust API
([spec §13](../spec/13-compatibility-and-versioning.md)):
...
Both are spelled out in [spec §13](../spec/13-compatibility-and-versioning.md);
neither is negotiable in a release without editing that section first.

## Before publishing

1. The four checks are clean ([workflows](workflows.md)).
2. `spec/15-open-tasks.md` has no task that this release claims to close.
3. Any change to `Policy::patient_identifiers` is reflected in
   [spec §5.1](../spec/05-built-in-policies.md) and named in the release
   notes.
