...
choice. Call your format's own function:
`serde_json::to_string(&message)`, `serde_yaml::to_string(&message)`,
...
So the round trip holds: `Message::parse(text)?` through JSON (or any
other format) and back out through `.to_er7()` reproduces `text` (modulo
the same normalization `er7::parse` alone would already do). Decoding is
lossy — a formatting escape like `\.br\` has no plain-text form to decode
back to — so a wire format built on decoded text could not, in general,
be turned back into valid ER7. If you want the decoded, human-readable
value, read it from the message after deserializing:
`message.query("OBX-3.2")?`, the same call you'd make against plain
`er7`. See [`spec/04-round-trip-guarantee.md`](../../spec/04-round-trip-guarantee.md).
...
See [`spec/02-wire-shapes.md`](../../spec/02-wire-shapes.md) §2.2.
...
repetition, containing one component, containing one subcomponent), not
one or two. See
...
`examples/inspect_a_segment_as_json.rs`) rather than writing the nesting
...
[`spec/05-error-handling.md`](../../spec/05-error-handling.md) §5.2.
...
[`spec/01-purpose-and-scope.md`](../../spec/01-purpose-and-scope.md) §1.2.

## Is this crate 1.0 yet? Can the wire shape change?

Not yet — see [`spec/08-versioning-and-compatibility.md`](../../spec/08-versioning-and-compatibility.md)
